...
class DataGenerator:
    ...
    def generate_normal_transactions(
        self, n_wallets: int = 100, n_transactions: int = 500
    ) -> pd.DataFrame:
        """
    ...
    def generate_layered_pattern(
        self, n_layers: int = 3, wallets_per_layer: int = 5
    ) -> pd.DataFrame:
        """
    ...
    def generate_complete_dataset(self, output_dir: str = "."):
        ...
        normal_df, _ = self.generate_normal_transactions(
        ...
        for _ in range(5):
            pattern_df, sources, _, _ = self.generate_fanout_fanin_pattern(
        ...
        for _ in range(3):
            pattern_df, sources, _, _ = self.generate_layered_pattern(
        ...
        # Normalize to the pipeline schema.
        pipeline_df = final_df.rename(
            columns={
        ...
        pipeline_df = pipeline_df[["src_wallet", "dst_wallet", "amount", "timestamp"]]
        pipeline_df["timestamp"] = pipeline_df["timestamp"].astype("int64") // 10**9

        data_dir = os.path.join(output_dir, "data")
        os.makedirs(data_dir, exist_ok=True)

        transactions_file = os.path.join(data_dir, "transactions.csv")
        pipeline_df.to_csv(transactions_file, index=False)
        print(f"\nTransactions saved to {transactions_file}")
        print(f"  Total transactions: {len(pipeline_df)}")
        print(
            "  Total wallets: "
            f"{len(set(pipeline_df['src_wallet']) | set(pipeline_df['dst_wallet']))}"
        )
        ...
        illicit_file = os.path.join(data_dir, "illicit_wallets.csv")
