...
def load_illicit_seeds(csv_path, fallback):
    if not os.path.exists(csv_path):
        print(f"Warning: illicit seed file not found at {csv_path}. Using defaults.")
        return fallback

    try:
        df = pd.read_csv(csv_path)
    except Exception as exc:
        print(f"Warning: failed to read {csv_path} ({exc}). Using defaults.")
        return fallback
    ...
    print(f"Warning: no wallet column found in {csv_path}. Using defaults.")
    return fallback
...
def main():
    ...
    if illicit_seeds and all(seed not in G for seed in illicit_seeds):
        print("Warning: none of the illicit seeds are present in the graph.")
    ...
    print("  - output/laundering_graph.html (interactive visualization)")
    print("  - output/suspicion_report.csv (detailed scores)")
